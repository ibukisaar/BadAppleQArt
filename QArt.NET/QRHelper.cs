...
namespace QArt.NET {
    public static class QRHelper {
        ...
        /// [Version, ECLevel]
        /// </summary>
        static readonly (int EccPerBlock, int BlocksInGroup1, int CodewordsInGroup1, int BlocksInGroup2, int CodewordsInGroup2)[,] blockTable = {
            { // Version 1
                (    7,    1,   19,    0,    0), // ECC = L
                (   10,    1,   16,    0,    0), // ECC = M
                (   13,    1,   13,    0,    0), // ECC = Q
        ...
        public const int MaxDataLength = 7089;
        public const int MaxEncodedBits = 2956 * 8;


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static int GetIndex(QREcLevel ecLevel) => (int)ecLevel ^ 1;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetSize(int version) => (version << 2) + 17;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetDataCapacity(int version, QREcLevel ecLevel) {
            return dataCapacityTable[version - 1, GetIndex(ecLevel)].NumberOfDataBytes;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref readonly (int EccPerBlock, int BlocksInGroup1, int CodewordsInGroup1, int BlocksInGroup2, int CodewordsInGroup2) GetBlockInfo(int version, QREcLevel ecLevel) {
            return ref blockTable[version - 1, GetIndex(ecLevel)];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int GetFormatBits(QREcLevel ecLevel, QRMaskVersion maskVersion)
            => FormatTable.Table[((int)ecLevel << 3) | (int)maskVersion];

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int GetVersionBits(int version)
            => VersionTable.Table[version];

        public static int GetTotalEccBytes(int version, QREcLevel ecLevel) {
            var (eccBytes, blocks1, _, blocks2, _) = GetBlockInfo(version, ecLevel);
        ...
        static class FormatTable {
            public readonly static UnmanagedArray<int> Table = new(length: 4 * 8);

            static FormatTable() {
                for (int ecLevel = 0; ecLevel < 4; ecLevel++) {
                    for (int maskVersion = 0; maskVersion < 8; maskVersion++) {
                        const int Generator = 0b10100110111, Mask = 0b101010000010010;
                        int formatIndex = (ecLevel << 3) | maskVersion;
                        int format = formatIndex << 10;
                        int bch = GF.PolynomMod(format, Generator);
                        Table[formatIndex] = (format | bch) ^ Mask;
                    }
                }
            }
        }

        static class VersionTable {
            public readonly static UnmanagedArray<int> Table = new(length: 41);

            static VersionTable() {
                for (int ver = 1; ver <= 40; ver++) {
                    const int Generator = 0b1111100100101;
                    int versionBits = ver << 12;
                    versionBits |= GF.PolynomMod(versionBits, Generator);
                    Table[ver] = versionBits;
                }
            }
        }
    }
